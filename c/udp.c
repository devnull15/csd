...
// Same as send and recieve wrapper but uses sendto and recvfrom instead.
//sendto and recvfrom aren't really necessary because the sockets are made in the same
//program but this is just for demo purposes
int storfr_wrapper(char *msg, char cs, char sr, int s) {
  char *name = (cs==SERVER) ? "Server" : "Client";

  if(sr==SEND){
    printf("%s %i is sending: '%s'\n", name,s,msg);
    //send_addr is ignored in a SOCK_STREAM and I'm not making a UDP socket
    if(sendto(s, msg, strlen(msg) , 0,NULL,0)<0) {
      printf("%s %i error in sendto.\n",name,s);
      perror("*");
      return EXIT_FAILURE;
    }
  }
    ...
    //recv from actually doesn't populate the sockaddr_in correctly if its a TCP socket
    //going to change to TCP
    if(recvfrom(s, (void*)buf, buf_size,0,(struct sockaddr*)&from,&addrlen)<0) {
      printf("%s %i error in recvfrom.\n",name,s);
      perror("*");
      return EXIT_FAILURE;
    }
    char from_addr[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &from.sin_addr,from_addr, INET_ADDRSTRLEN);
    printf("%s %i received message from %s: '%s'\n",name,s,from_addr,buf);
  }
  else {
